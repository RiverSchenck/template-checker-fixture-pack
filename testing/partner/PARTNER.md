...
3. Implement `normalizeToIssues` — walk your checker’s native result shape (see the fictional `EXAMPLE_NATIVE_RESULT` in `binding.example.js`) or use `normalizeFlatIssueRows` if you already emit `[{ checkId, type }, …]`.
...
When prompted, choose the **`Plugin_test_packages`** folder (parent of `unit/`). If you received the fixture-pack repository, use its `Plugin_test_packages/` directory at the repo root.
